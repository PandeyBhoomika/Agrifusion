...
const CATEGORIES = [
  ...
  { id: "income-support",      label: "Income",       emoji: "🪙" },
...
// ─── Helper: display eligibility cleanly ─────────────────────────────────────

const eligibilityText = (scheme: GovernmentScheme): string => {
  if (Array.isArray(scheme.eligibility)) {
    return scheme.eligibility.join(" • ");
  }
  return scheme.eligibility || "";
};

// ─── Helper: display scheme name (handles both backend & fallback field names)

const schemeName = (scheme: GovernmentScheme): string =>
  scheme.title || scheme.name || "";

// ─── Helper: display apply link ──────────────────────────────────────────────

const applyLink = (scheme: GovernmentScheme): string =>
  scheme.link || scheme.applicationLink || "https://agricoop.nic.in/";
...
export default function GovernmentSchemesScreen() {
  ...
  const loadSchemes = async () => {
    try {
      const data = await fetchGovernmentSchemes(); // reads userState from AsyncStorage
      setAllSchemes(data);
      // Detect if we got real API data or fallback
      const isRealData = data.length > 0 && !data[0].id.startsWith("fb");
      setSource(isRealData ? "api" : "fallback");
    } catch (error) {
  ...
  useEffect(() => {
    let result = filterSchemesByCategory(allSchemes, selectedCategory);
    result = searchSchemes(result, searchQuery);
  ...
  const handleApply = async (scheme: GovernmentScheme) => {
    const url = applyLink(scheme);
    try {
      const canOpen = await Linking.canOpenURL(url);
      if (canOpen) await Linking.openURL(url);
      else await Linking.openURL("https://agricoop.nic.in/");
    } catch {
  ...
  return (
    <LinearGradient colors={["#021F0F", "#042818", "#053B24"]} style={styles.mainContainer}>
      ...
      <SafeAreaView style={styles.safe}>
        ...
        {/* Data source banner — shown only when using fallback */}
        {source === "fallback" && (
          <View style={styles.fallbackBanner}>
            <Ionicons name="wifi-outline" size={14} color="#fde68a" />
            <Text style={styles.fallbackText}>
              Showing offline data. Connect to backend to see full schemes list.
            </Text>
          </View>
        )}
        ...
        {loading ? (
          ...
          >
            ...
            {filteredSchemes.map((scheme, index) => {
              const id = scheme.id || scheme._id || String(index);
              const isExpanded = expandedSchemeId === id;
              const displayName = schemeName(scheme);
              const displayEligibility = eligibilityText(scheme);

              return (
                <Animated.View key={id} entering={FadeInUp.delay(index * 50).duration(400)}>
                  <TouchableOpacity
                        ...
                        <View style={styles.badgeRow}>
                          <View style={styles.categoryBadge}>
                            <Text style={styles.categoryBadgeText}>
                              {scheme.category.replace(/-/g, " ").toUpperCase()}
                            </Text>
                          </View>
                          {/* State badge */}
                          <View style={[styles.stateBadge, scheme.state === "National" && styles.stateBadgeNational]}>
                            <Text style={styles.stateBadgeText}>{scheme.state}</Text>
                          </View>
                          {(scheme.status || (scheme.isActive !== false)) && (
                            <View style={styles.activeBadge}>
                              <Text style={styles.activeBadgeText}>{t.schemes.active}</Text>
                            </View>
                          )}
                        </View>
                        <Text style={styles.schemeName}>{displayName}</Text>
                      </View>
                      <Ionicons
                          ...
                          {Array.isArray(scheme.eligibility) ? (
                            scheme.eligibility.map((item, i) => (
                              <View key={i} style={styles.eligibilityRow}>
                                <Ionicons name="checkmark-circle" size={14} color="#22c55e" style={{ marginRight: 6, marginTop: 2 }} />
                                <Text style={styles.detailValue}>{item}</Text>
                              </View>
                            ))
                          ) : (
                            <Text style={styles.detailValue}>{displayEligibility}</Text>
                          )}
                        </View>
...
const styles = StyleSheet.create({
  ...
  // Fallback banner
  fallbackBanner: { marginHorizontal: 20, marginBottom: 8, backgroundColor: "rgba(251,191,36,0.12)", borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8, flexDirection: "row", alignItems: "center", gap: 8, borderWidth: 1, borderColor: "rgba(251,191,36,0.25)" },
  fallbackText: { color: "#fde68a", fontSize: 12, flex: 1 },
  ...
  resultCount: { color: "rgba(255,255,255,0.4)", fontSize: 12, marginBottom: 12, marginTop: 4 },
  ...
  stateBadge: { backgroundColor: "rgba(99,102,241,0.15)", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  stateBadgeNational: { backgroundColor: "rgba(59,130,246,0.15)" },
  stateBadgeText: { color: "#a5b4fc", fontSize: 9, fontWeight: "700" },
  ...
  detailLabel: { color: "#9ca3af", fontSize: 11, fontWeight: "700", textTransform: "uppercase", marginBottom: 6 },
  detailValue: { color: "#e5e7eb", fontSize: 13, lineHeight: 18, flex: 1 },
  eligibilityRow: { flexDirection: "row", alignItems: "flex-start", marginBottom: 4 },
