...
import {
  ...
  Modal, StyleSheet, Platform, ActivityIndicator, TextInput,
} from 'react-native';
...
interface LearningModule {
  ...
  difficulty: Difficulty; category: string; type: ModuleType; thumbnail: string;
  completed: boolean; progress: number; points: number; locked: boolean; prerequisites?: string[];
}

interface FarmPlot {
  id: string; x: number; y: number; currentCrop: any | null;
  soilHealth: { nitrogen: number; phosphorus: number; potassium: number };
  pestPressure: number; yield: number;
}

interface GameLevel {
  id: number; name: string; goal: string; targetScore: number; plotCount: number; seasons: number;
}

interface LearningHubProps {
  onBack?: () => void; onViewVirtualFarm?: () => void; onViewDashboard?: () => void;
  onViewWeeklyTasks?: () => void; autoDemo?: boolean; onMiniGames?: () => void;
}

const learningModules: LearningModule[] = [
  { id: 'irrigation-basics', title: 'Smart Irrigation Systems', description: 'Efficient water management & irrigation methods', duration: '12 min', difficulty: 'beginner', category: 'irrigation', type: 'simulation', thumbnail: '', completed: true, progress: 100, points: 150, locked: false },
  { id: 'crop-rotation-basics', title: 'Sustainable Crop Rotation', description: 'Crop rotation for soil health and yields', duration: '8 min', difficulty: 'beginner', category: 'crop-rotation', type: 'video', thumbnail: '', completed: true, progress: 100, points: 120, locked: false },
  { id: 'pest-control-organic', title: 'Organic Pest Management', description: 'Protect crops without harmful chemicals', duration: '15 min', difficulty: 'intermediate', category: 'pest-control', type: 'simulation', thumbnail: '', completed: false, progress: 60, points: 200, locked: false },
];

const irrigationSimulation = [
  { id: 'step-1', title: 'Check Soil Moisture', description: 'Tap the soil sensor to check moisture', action: 'tap' as const, target: 'soil-sensor', feedback: 'Soil moisture is 25% — needs watering.', points: 20 },
  { id: 'step-2', title: 'Select Irrigation Zone', description: 'Choose the irrigation zone', action: 'select' as const, target: 'zone-selector', feedback: 'Zone A selected — lowest moisture.', points: 30 },
  { id: 'step-3', title: 'Set Water Duration', description: 'Choose duration (recommended 15-20 min)', action: 'drag' as const, target: 'time-slider', feedback: '18 minutes chosen — optimal.', points: 40 },
  { id: 'step-4', title: 'Start Irrigation', description: 'Start watering', action: 'tap' as const, target: 'start-button', feedback: 'Water is flowing efficiently.', points: 50 },
];

const userProgress = { totalPoints: 650, completedModules: 3, totalModules: 6, currentStreak: 5, level: 3, badges: ['First Steps', 'Water Wise', 'Crop Expert'], weeklyGoal: 4, weeklyCompleted: 2 };
...
export default function LearningHub({ onBack, onViewVirtualFarm, onViewDashboard, onViewWeeklyTasks, autoDemo = false, onMiniGames }: LearningHubProps) {
  ...
  useEffect(() => {
    if (autoDemo && onMiniGames) { const timer = setTimeout(() => onMiniGames(), 2500); return () => clearTimeout(timer); }
  ...
  useEffect(() => {
    const loadSavedProgress = async () => {
      if (selectedVideo) {
        setIsLoadingProgress(true); setProgressLoaded(false); setShouldSeek(false); setPlaying(false);
        try {
          const savedProgress = await getVideoProgress(selectedVideo.id);
          if (savedProgress && savedProgress.currentTime > 0) {
            setVideoProgress(savedProgress.progress); setCurrentTime(savedProgress.currentTime);
            setVideoDuration(savedProgress.duration); setShouldSeek(true);
          } else { setVideoProgress(0); setCurrentTime(0); setVideoDuration(0); setShouldSeek(false); }
    ...
    const interval = setInterval(async () => {
      if (youtubePlayerRef.current) {
        try {
          ...
          if (duration > 0 && current > 0) { setVideoDuration(duration); setCurrentTime(current); setVideoProgress(Math.round((current / duration) * 100)); }
        } catch (e) {}
      }
    }, 1000);
  ...
  useEffect(() => {
    ...
    const saveInterval = setInterval(() => {
      if (currentTime > 0) { const isCompleted = videoProgress >= 100; updateVideoProgress(selectedVideo.id, videoProgress, currentTime, videoDuration, isCompleted); }
    }, 5000);
  ...
  const handleModuleSelect = useCallback((module: LearningModule) => {
    if (!module.locked) {
      setSelectedModule(module); setSimulationStep(0); setSimulationScore(0); setSimulationComplete(false);
      if (module.type === 'video') { const videoData = videos.find(v => v.category === module.category); setSelectedVideo(videoData || null); }
    }
  }, [videos]);
  ...
  const handleSimulationAction = useCallback((stepId: string) => {
    ...
    if (current && current.id === stepId) {
      ...
      if (simulationStep < irrigationSimulation.length - 1) { setSimulationStep((s) => s + 1); }
      else { setSimulationComplete(true); }
    }
  }, [simulationStep]);
  ...
  const baseData = selectedTab === 'videos' ? videos : learningModules.filter((m) =>
    selectedTab === 'overview' || (selectedTab === 'simulations' && m.type === 'simulation') || (selectedTab === 'quizzes' && m.type === 'quiz')
  );

  const displayData = baseData.filter((item: any) => item.title.toLowerCase().includes(searchQuery.toLowerCase()));

  // ✅ Translated tab labels map
  ...
  if (selectedModule) {
            ...
            <Animated.View entering={FadeInUp.delay(100).duration(400)}>
              {selectedModule.type === 'simulation' && (
                <View style={styles.card}>
                  {/* ✅ Translated */}
                  <Text style={styles.cardTitle}>{t.learningHub.simulationComingSoon}</Text>
                  <Text style={styles.cardText}>{t.learningHub.simulationText}</Text>
                  <TouchableOpacity style={styles.primaryButton} onPress={() => setSelectedModule(null)}>
                    <Text style={styles.primaryButtonText}>{t.learningHub.backToHub}</Text>
                  </TouchableOpacity>
                </View>
              )}
                ...
                <View style={styles.card}>
                  {isLoadingProgress && (
                    <View style={{ padding: 40, alignItems: 'center' }}>
                      ...
                      {/* ✅ Translated */}
                      <Text style={{ marginTop: 16, color: '#166534', fontWeight: '600' }}>{t.learningHub.loadingVideo}</Text>
                    </View>
                  )}
                  {!isLoadingProgress && progressLoaded && (
                    <>
                      ...
                      <View style={styles.videoContainer}>
                        <YoutubePlayer
                          ref={youtubePlayerRef} height={220} play={playing}
    ...
    if (state === 'ended') {
      ...
      setVideoProgress(100);
                          ...
                          onReady={() => { if (shouldSeek && currentTime > 0) { setTimeout(() => { youtubePlayerRef.current?.seekTo(currentTime, true); setShouldSeek(false); }, 500); } }}
                        />
                      </View>
            ...
            <>
              ...
              <Animated.View entering={FadeInUp.delay(300).duration(400)} style={styles.progressCard}>
                ...
                <View style={{ marginTop: 12 }}>
                  {/* ✅ Translated */}
                  <Text style={{ fontSize: 12, color: '#166534', fontWeight: '600', marginBottom: 4 }}>{t.learningHub.courseCompletion}</Text>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tabScrollContainer}>
                  {(['overview', 'simulations', 'videos', 'quizzes', 'mini-games'] as const).map((tab) => (
                    <TouchableOpacity
                      ...
                      onPress={() => setSelectedTab(tab)} activeOpacity={0.75}
                    >
                      {/* ✅ Translated */}
          ...
          renderItem={({ item, index }) => {
            ...
            return (
              <Animated.View entering={FadeInUp.delay(500 + (index * 100)).duration(400)}>
                <TouchableOpacity
                  style={styles.moduleCard} activeOpacity={0.75}
                  onPress={() => {
                    if (isVideo && videoItem) {
                      setSelectedVideo(videoItem);
                      setSelectedModule({ id: videoItem.id, title: videoItem.title, description: videoItem.description, duration: videoItem.duration, difficulty: videoItem.difficulty, category: videoItem.category, type: 'video', thumbnail: videoItem.thumbnail, completed: videoItem.completed, progress: videoItem.progress, points: videoItem.points, locked: false });
                    } else if (moduleItem) { handleModuleSelect(moduleItem); }
                  }}
                >
                  ...
                  <View style={{ flex: 1, padding: 16 }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 }}>
                      <View style={styles.moduleMetaPill}>
                        <Text style={styles.moduleMetaText}>{isVideo ? `Video • ${videoItem?.duration}` : `${moduleItem?.type} • ${moduleItem?.duration}`}</Text>
                      </View>
